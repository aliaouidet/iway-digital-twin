...
@Component({
  ...
  template: `
    <div class="flex h-[calc(100vh-8rem)] gap-6">
      <!-- Chat Area -->
      <div class="flex-1 flex flex-col bg-white dark:bg-[#0F172A] rounded-2xl border border-slate-200 dark:border-slate-800 overflow-hidden shadow-sm dark:shadow-none">
        <!-- Chat Header -->
        <div class="px-6 py-4 border-b border-slate-200 dark:border-slate-800 flex items-center justify-between">
          <div class="flex items-center gap-3">
            ...
            <div>
              <h3 class="text-sm font-bold text-slate-900 dark:text-white">I-Sante AI Assistant</h3>
          ...
          <button (click)="clearChat()" class="px-3 py-1.5 bg-slate-100 dark:bg-slate-800 hover:bg-slate-200 dark:hover:bg-slate-700 border border-slate-200 dark:border-slate-700 rounded-lg text-xs text-slate-600 dark:text-slate-400 hover:text-slate-900 dark:hover:text-white transition-colors cursor-pointer">
            Clear
          </button>
        </div>
        ...
        <div #messagesContainer class="flex-1 overflow-y-auto px-6 py-6 space-y-6 custom-scrollbar">
          ...
          <div *ngIf="messages().length === 0 && !isThinking()" class="flex flex-col items-center justify-center h-full text-center">
            ...
            <h3 class="text-lg font-bold text-slate-900 dark:text-white mb-2" style="font-family: 'Figtree', sans-serif;">Ask I-Sante anything</h3>
            ...
            <div class="grid grid-cols-2 gap-2 mt-6 max-w-sm">
              <button *ngFor="let q of quickQuestions" (click)="sendQuickQuestion(q)"
                class="px-3 py-2 bg-slate-50 dark:bg-slate-800/60 border border-slate-200 dark:border-slate-700/50 hover:border-indigo-400 dark:hover:border-indigo-500/50 rounded-xl text-xs text-slate-600 dark:text-slate-400 hover:text-indigo-600 dark:hover:text-white transition-all cursor-pointer text-left shadow-sm dark:shadow-none">
                {{q}}
              </button>
            </div>
          </div>
          ...
          <div *ngIf="connectionError()" class="flex flex-col items-center justify-center h-full text-center">
            <div class="w-16 h-16 bg-red-500/10 rounded-2xl flex items-center justify-center mb-5">
              <svg class="w-8 h-8 text-red-500 dark:text-red-400" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="1.5" d="M12 9v3.75m9-.75a9 9 0 11-18 0 9 9 0 0118 0zm-9 3.75h.008v.008H12v-.008z"/></svg>
            </div>
            <h3 class="text-lg font-bold text-slate-900 dark:text-white mb-2">Connection Error</h3>
            <p class="text-sm text-red-500 dark:text-red-400/80 max-w-md">{{connectionError()}}</p>
          ...
          <div *ngFor="let msg of messages()" class="flex" [class]="msg.role === 'user' ? 'justify-end' : 'justify-start'">
            <div class="max-w-[70%] group">
              <!-- User Message -->
              <div *ngIf="msg.role === 'user'" class="bg-indigo-600 px-5 py-3 rounded-2xl rounded-br-md shadow-sm">
              ...
              <div *ngIf="msg.role === 'assistant'" class="flex items-start gap-3">
                <div class="w-7 h-7 rounded-lg bg-indigo-500/15 flex items-center justify-center flex-shrink-0 mt-1">
                  <svg class="w-3.5 h-3.5 text-indigo-600 dark:text-indigo-400" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M9.813 15.904L9 18.75l-.813-2.846a4.5 4.5 0 00-3.09-3.09L2.25 12l2.846-.813a4.5 4.5 0 003.09-3.09L9 5.25"/></svg>
                </div>
                <div class="bg-slate-50 dark:bg-slate-800/60 px-5 py-3 rounded-2xl rounded-bl-md border border-slate-200 dark:border-slate-700/50 shadow-sm dark:shadow-none">
                  <p class="text-sm text-slate-700 dark:text-slate-200 leading-relaxed whitespace-pre-wrap">{{msg.content}}<span *ngIf="msg.isStreaming" class="inline-block w-1.5 h-4 bg-indigo-500 dark:bg-indigo-400 animate-pulse ml-0.5 align-text-bottom rounded-sm"></span></p>
                  <div *ngIf="msg.processor" class="mt-2 flex items-center gap-2">
                    <span [class]="getProcessorBadge(msg.processor)">{{msg.processor}}</span>
                  </div>
                </div>
              </div>
              <div class="text-[10px] text-slate-500 dark:text-slate-600 mt-1" [class]="msg.role === 'user' ? 'text-right' : 'text-left pl-10'">{{msg.timestamp}}</div>
            </div>
          </div>
          ...
          <div *ngIf="currentStreamingMessage()" class="flex justify-start">
            <div class="max-w-[70%] group">
              <div class="flex items-start gap-3">
                ...
                <div class="bg-slate-50 dark:bg-slate-800/60 px-5 py-3 rounded-2xl rounded-bl-md border border-slate-200 dark:border-slate-700/50 shadow-sm dark:shadow-none">
                  <p class="text-sm text-slate-700 dark:text-slate-200 leading-relaxed whitespace-pre-wrap">{{currentStreamingMessage()}}<span class="inline-block w-1.5 h-4 bg-indigo-500 dark:bg-indigo-400 animate-pulse ml-0.5 align-text-bottom rounded-sm"></span></p>
                </div>
              </div>
            </div>
          </div>
          ...
          <div *ngIf="isThinking()" class="flex justify-start">
            <div class="flex items-center gap-3 bg-slate-50 dark:bg-slate-800/40 px-4 py-2.5 rounded-full border border-slate-200 dark:border-slate-700/50 shadow-sm dark:shadow-none">
               <svg class="w-4 h-4 text-indigo-500 dark:text-indigo-400 animate-spin" fill="none" viewBox="0 0 24 24"><circle class="opacity-25" cx="12" cy="12" r="10" stroke="currentColor" stroke-width="4"></circle><path class="opacity-75" fill="currentColor" d="M4 12a8 8 0 018-8V0C5.373 0 0 5.373 0 12h4z"></path></svg>
               <span class="text-xs text-slate-500 dark:text-slate-400">{{thinkingStatus() || 'Processing...'}}</span>
            </div>
          </div>
        ...
        <div class="p-4 border-t border-slate-200 dark:border-slate-800 bg-white dark:bg-transparent">
          <form (ngSubmit)="sendMessage()" class="flex gap-3 items-end">
            ...
            <button type="submit" [disabled]="!newMessage.trim() || isSending() || !isSessionReady()"
              class="w-11 h-11 bg-indigo-600 hover:bg-indigo-500 disabled:bg-slate-200 dark:disabled:bg-slate-800 disabled:text-slate-400 dark:disabled:text-slate-600 rounded-xl flex items-center justify-center text-white transition-all cursor-pointer disabled:cursor-not-allowed flex-shrink-0 shadow-sm">
      ...
      <div class="w-72 space-y-4 hidden xl:block">
        <div class="bg-white dark:bg-[#0F172A] rounded-2xl border border-slate-200 dark:border-slate-800 p-5 shadow-sm dark:shadow-none">
          <h3 class="text-xs font-bold text-slate-500 dark:text-slate-400 uppercase tracking-wider mb-4">Session Info</h3>
          <div class="space-y-3">
            <div class="flex justify-between text-sm"><span class="text-slate-500">Messages</span><span class="text-slate-900 dark:text-white font-semibold">{{messages().length}}</span></div>
            <div class="flex justify-between text-sm"><span class="text-slate-500">Pipeline</span><span class="text-emerald-500 dark:text-emerald-400 font-semibold text-xs">RAG → LLM → Human</span></div>
            <div class="flex justify-between text-sm"><span class="text-slate-500">Model</span><span class="text-slate-700 dark:text-slate-300 font-semibold text-xs">Gemini 2.5 Flash</span></div>
          </div>
        </div>
        <div class="bg-white dark:bg-[#0F172A] rounded-2xl border border-slate-200 dark:border-slate-800 p-5 shadow-sm dark:shadow-none">
          <h3 class="text-xs font-bold text-slate-500 dark:text-slate-400 uppercase tracking-wider mb-4">Processing Pipeline</h3>
          <div class="space-y-3">
            <div class="flex items-center gap-3">
              <div class="w-8 h-8 rounded-lg bg-emerald-500/15 flex items-center justify-center"><span class="text-emerald-500 dark:text-emerald-400 text-xs font-bold">1</span></div>
              <div>
                <div class="text-xs font-semibold text-slate-900 dark:text-white">RAG Search</div>
                <div class="text-[10px] text-slate-500">FAISS vector similarity</div>
              </div>
            </div>
            <div class="w-px h-3 bg-slate-200 dark:bg-slate-700 ml-4"></div>
            <div class="flex items-center gap-3">
              <div class="w-8 h-8 rounded-lg bg-indigo-500/15 flex items-center justify-center"><span class="text-indigo-500 dark:text-indigo-400 text-xs font-bold">2</span></div>
              <div>
                <div class="text-xs font-semibold text-slate-900 dark:text-white">LLM Generation</div>
                <div class="text-[10px] text-slate-500">Gemini / Qwen fallback</div>
              </div>
            </div>
            <div class="w-px h-3 bg-slate-200 dark:bg-slate-700 ml-4"></div>
            <div class="flex items-center gap-3">
              <div class="w-8 h-8 rounded-lg bg-amber-500/15 flex items-center justify-center"><span class="text-amber-500 dark:text-amber-400 text-xs font-bold">3</span></div>
              <div>
                <div class="text-xs font-semibold text-slate-900 dark:text-white">Human Escalation</div>

  ...
  template: `
    <div class="space-y-6">
      <div class="flex items-center justify-between">
        <div>
          <h1 class="text-2xl font-bold text-slate-900 dark:text-white tracking-tight" style="font-family: 'Figtree', sans-serif;">Tickets & Alerts</h1>
          <p class="text-slate-500 dark:text-slate-400 mt-1 text-sm">Monitor escalated requests and RAG processing pipeline</p>
        </div>
      </div>
      ...
      <div *ngIf="isLoading()" class="space-y-4">
        <div *ngFor="let _ of [1,2,3]" class="bg-white dark:bg-slate-800/50 rounded-2xl border border-slate-200 dark:border-slate-700/50 p-6 animate-pulse shadow-sm dark:shadow-none">
      ...
      <div *ngIf="!isLoading()" class="flex gap-6">
        ...
        <div class="flex-1 space-y-3 max-h-[70vh] overflow-y-auto pr-2 custom-scrollbar">
          ...
          <div *ngFor="let ticket of filteredTickets(); let i = index"
            ...
            [class]="selectedTicket() === ticket
              ...
              : 'bg-white dark:bg-[#0F172A] p-5 rounded-2xl border border-slate-200 dark:border-slate-800 hover:border-slate-300 dark:hover:border-slate-600 hover:shadow-md dark:hover:shadow-none cursor-pointer transition-all shadow-sm dark:shadow-none'">
            <div class="flex items-start gap-4">
              <div [class]="getStatusIconClass(ticket.outcome)">
                <svg *ngIf="isRagResolved(ticket.outcome)" class="w-4 h-4" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M5 13l4 4L19 7"/></svg>
              ...
              <div class="flex-1 min-w-0">
                <div class="flex items-center justify-between mb-1">
                  <h4 class="text-sm font-semibold text-slate-900 dark:text-white truncate pr-4">{{ticket.query}}</h4>
        ...
        <!-- Detail Panel -->
        <div class="w-96 bg-white dark:bg-[#0F172A] rounded-2xl border border-slate-200 dark:border-slate-800 p-6 max-h-[70vh] overflow-y-auto custom-scrollbar sticky top-0 shadow-sm dark:shadow-none" *ngIf="selectedTicket()">
          <h3 class="text-base font-bold text-slate-900 dark:text-white mb-5" style="font-family: 'Figtree', sans-serif;">Request Details</h3>

          <div class="space-y-5">
            <div>
              <label class="text-[10px] font-semibold text-slate-500 uppercase tracking-wider">Query</label>
              <p class="text-sm text-slate-700 dark:text-slate-200 mt-1 leading-relaxed">{{selectedTicket()!.query}}</p>
            </div>

            <div class="grid grid-cols-2 gap-4">
              <div>
                <label class="text-[10px] font-semibold text-slate-500 uppercase tracking-wider">Model</label>
                <p class="text-sm text-slate-700 dark:text-slate-300 mt-1">{{selectedTicket()!.model}}</p>
              </div>
              <div>
                <label class="text-[10px] font-semibold text-slate-500 uppercase tracking-wider">Tokens Used</label>
                <p class="text-sm text-slate-700 dark:text-slate-300 mt-1">{{selectedTicket()!.tokens_used}}</p>
              </div>
            </div>
            ...
            <div>
              ...
              <div class="mt-2 flex items-center gap-3">
                ...
                <span class="text-sm font-bold text-indigo-500 dark:text-indigo-400">{{(selectedTicket()!.top_similarity * 100).toFixed(0)}}%</span>
              </div>
            </div>
            ...
            <div class="grid grid-cols-2 gap-4">
              <div>
                ...
                <p class="text-sm text-slate-700 dark:text-slate-300 mt-1">{{selectedTicket()!.chunks_retrieved}}</p>
              </div>
              <div>
                <label class="text-[10px] font-semibold text-slate-500 uppercase tracking-wider">Gen Time</label>
                <p class="text-sm text-slate-700 dark:text-slate-300 mt-1">{{selectedTicket()!.gen_time_ms}}ms</p>
              </div>
            </div>
...
export class TicketsComponent implements OnInit, OnDestroy {
  ...
  filteredTickets = computed(() => {
    ...
    if (filter === 'RAG_RESOLVED') return all.filter(t => this.isRagResolved(t.outcome));
      ...
      this.wsService.getTraceUpdates().subscribe(trace => {
        if (trace && trace.query) {
          const newEntry: LogEntry = {
            ...
            confidence: trace.confidence <= 1 && trace.confidence > 0 ? Math.round(trace.confidence * 100) : Math.round(trace.confidence || 0),
  ...
  private buildFilterTabs(): void {
    ...
    this.filterTabs = [
      ...
      { label: 'RAG Resolved', value: 'RAG_RESOLVED', count: () => all().filter(t => this.isRagResolved(t.outcome)).length },
  ...
  isRagResolved(outcome: string): boolean {
    return ['RAG_RESOLVED', 'GRAPH_RESOLVED', 'STALL_RESOLVED'].includes(outcome);
  }

  formatOutcome(outcome: string): string {
    if (this.isRagResolved(outcome)) return 'RAG ✓';
  ...
  getStatusIconClass(outcome: string): string {
    ...
    if (this.isRagResolved(outcome)) return base + 'bg-emerald-500/15 text-emerald-400';
    const map: Record<string, string> = {
  ...
  getStatusBadgeClass(outcome: string): string {
    ...
    if (this.isRagResolved(outcome)) return base + 'bg-emerald-500/10 text-emerald-400 border border-emerald-500/20';
    const map: Record<string, string> = {

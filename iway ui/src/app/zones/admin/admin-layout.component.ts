...
@Component({
  ...
  template: `
    <div class="flex h-screen overflow-hidden transition-colors duration-300 relative bg-slate-50 text-slate-800 dark:bg-[#020617] dark:text-slate-200">
      ...
      <!-- Sidebar -->
      <aside class="w-64 flex flex-col border-r z-50 flex-shrink-0 transition-transform duration-300 ease-in-out absolute lg:relative lg:translate-x-0 h-full shadow-2xl lg:shadow-none bg-white border-slate-200 dark:bg-[#0F172A] dark:border-slate-800"
        [class.translate-x-0]="isSidebarOpen()"
        [class.-translate-x-full]="!isSidebarOpen()">
        <div class="p-5 border-b flex items-center justify-between flex-shrink-0 h-14 border-slate-200 dark:border-slate-800">
          <div style="width: 110px;">
            <app-iway-logo width="100%"></app-iway-logo>
          </div>
          <!-- Close button on mobile -->
          <button (click)="toggleSidebar()" class="lg:hidden p-1.5 -mr-1.5 rounded-lg transition-colors cursor-pointer hover:bg-slate-100 text-slate-500 dark:hover:bg-slate-800 dark:text-slate-400">
        ...
        <nav class="flex-1 px-3 py-4 space-y-2 overflow-y-auto custom-scrollbar">
          ...
          <!-- Monitoring Dropdown -->
          <div>
            <button (click)="toggleMonitoringDropdown()" class="w-full flex items-center justify-between px-3 py-2.5 rounded-lg transition-colors cursor-pointer hover:bg-slate-100/50 text-slate-700 dark:hover:bg-slate-800/50 dark:text-slate-300">
            ...
            <div [@dropdownAnimation]="isMonitoringDropdownOpen() ? 'open' : 'closed'" class="overflow-hidden px-2 mt-1">
              <a (click)="closeSidebar()" routerLink="/admin/dashboard" routerLinkActive="active-link"
                class="nav-link flex items-center gap-3 px-3 py-2 rounded-lg transition-all text-sm font-medium cursor-pointer border-l-2 border-transparent text-slate-500 hover:text-slate-900 hover:bg-slate-100 dark:text-slate-400 dark:hover:text-slate-200 dark:hover:bg-white/5">
          ...
          <!-- System Dropdown -->
          <div class="mt-2">
            <button (click)="toggleSystemDropdown()" class="w-full flex items-center justify-between px-3 py-2.5 rounded-lg transition-colors cursor-pointer hover:bg-slate-100/50 text-slate-700 dark:hover:bg-slate-800/50 dark:text-slate-300">
              <div class="flex items-center gap-3">
            ...
            <div [@dropdownAnimation]="isSystemDropdownOpen() ? 'open' : 'closed'" class="overflow-hidden px-2 mt-1">
              ...
              <a (click)="closeSidebar()" routerLink="/admin/knowledge" routerLinkActive="active-link"
                class="nav-link flex items-center gap-3 px-3 py-2 rounded-lg transition-all text-sm font-medium cursor-pointer border-l-2 border-transparent text-slate-500 hover:text-slate-900 hover:bg-slate-100 dark:text-slate-400 dark:hover:text-slate-200 dark:hover:bg-white/5">
                <div class="w-1.5 h-1.5 rounded-full bg-fuchsia-500 opacity-50 ml-1"></div>
                Knowledge
              </a>
        ...
        <!-- User Info -->
        <div class="p-4 border-t flex-shrink-0 border-slate-200 dark:border-slate-800">
          <div class="flex items-center gap-3 px-1">
            ...
            <div class="flex-1 min-w-0">
              <div class="text-sm font-semibold truncate text-slate-900 dark:text-white">{{userName()}}</div>
              <div class="text-xs truncate text-slate-400 dark:text-slate-500">{{userRole()}}</div>
            </div>
            <div class="flex items-center gap-1">
              <button (click)="toggleTheme()"
                class="p-2 rounded-lg transition-colors cursor-pointer hover:bg-slate-100 text-slate-400 dark:hover:bg-slate-800 dark:text-slate-500">
              ...
              <button (click)="logout()" class="p-2 rounded-lg transition-colors cursor-pointer hover:bg-slate-100 text-slate-400 hover:text-rose-500 dark:hover:bg-slate-800 dark:text-slate-500 dark:hover:text-rose-400" title="Logout">
        ...
        <header class="h-14 flex items-center px-4 lg:px-8 justify-between sticky top-0 z-20 border-b backdrop-blur-md flex-shrink-0 bg-white/80 border-slate-200 dark:bg-[#0F172A]/80 dark:border-slate-800">
          ...
          <div class="flex items-center space-x-3 lg:space-x-5">
            <div class="flex items-center gap-2">
              ...
              <span class="text-[10px] lg:text-xs font-medium whitespace-nowrap text-slate-500 dark:text-slate-400">RAG Engine Online</span>
            </div>
          </div>
        </header>
